...
public class Map {
	...
	public ArrayList<PointDir> buildCorridor(PointDir door){
		...
		while(!obstructed && length<8 && (rng.nextInt(7)<=5 || length <=3)){
			...
			if(isOpen(temp.x,temp.y) || end.point.x <= 1 || end.point.x >= height-1 || end.point.y <= 1 || end.point.y >= width-1){
				obstructed = true;
			}
			if(!obstructed){
	...
	public class Room {
		...
		Room(PointDir _door, boolean _start, RoomType type){
			...
			if (count<100) {
				...
				if(type.equals(RoomType.CORRIDOR)) makeCorridors();
			}else{
				valid = false;
			}
		}
		...
		public void makeCorridors(){
			...
			// Point bend = new Point(rng.nextInt(h-3)+2,rng.nextInt(w-3)+2);
			...
			pathList.add(door.point);

			Pathfinder.PointBFS path;

			for(int i = 0; i < localDoorPoints.length; i++){
				if(!opens[i]) continue;
				path = pf.pathfindBFS(new Point(door.point.x-x,door.point.y-y),localDoorPoints[i].point, room, false);
				while (path != null && path.getParent() != null) {
					pathList.add(path.getPos());
					path = path.getParent();
				}
				pathList.add(localDoorPoints[i].point);
			}

			// path from door to bend
			//			Point doorInRoom = new Point(door.point.x-x-1-1,door.point.y-y-1-1);
			//			
			//			pathList.add(doorInRoom);
			//			
			//			Pathfinder.PointBFS path = pf.pathfindBFS(doorInRoom, bend, room, false);
			//			while (path != null && path.getParent() != null) {
			//				pathList.add(path.getPos());
			//				path = path.getParent();
			//			}
			...
			//			pathList.add(bend);
			//			
			//			// path from bend to doors
			//			for(int i = 0; i < doorPoints.length; i++){
			//				if(!opens[i]) continue;
			//				Point thisDoorInRoom = new Point(doorPoints[i].point.x-x-1-1, doorPoints[i].point.y-y-1-1);
			//				path = pf.pathfindBFS(bend, thisDoorInRoom, room, false);
			//				while (path != null && path.getParent() != null) {
			//					pathList.add(path.getPos());
			//					path = path.getParent();
			//				}
			//				pathList.add(thisDoorInRoom);
			//			}
			...
			for(int _x = 0; _x < h; _x++){
				for(int _y = 0; _y < w; _y++){
					if(pathList.contains(new Point(_x,_y))){
						room[_x][_y] = 0;
					}else{
						room[_x][_y] = 1;
					}
