...
public class Pathfinder {
	...
	private Queue<PointBFS> q = new LinkedList<PointBFS>();
	...
	private boolean diagonalCheck(int dir, PointBFS p, Queue<PointBFS> q, int[][] temp){
		...
		int y = p.y;
		int blocks = 0;
		if((dir == 4 || dir == 5)&& !isOpen(p,null,x, y+1,temp)) blocks++;
		if((dir == 5 || dir == 6)&& !isOpen(p,null,x+1, y,temp)) blocks++;
		if((dir == 6 || dir == 7)&& !isOpen(p,null,x, y-1,temp)) blocks++;
		if((dir == 7 || dir == 4)&& !isOpen(p,null,x-1, y,temp)) blocks++;
